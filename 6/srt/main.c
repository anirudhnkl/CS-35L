...
int nthreads;
scene_t scene;
float vals [width][height][3];

void * renderImg(void * numThread)
{
    Vec3 camera_pos;
    ...
    /* for every pixel */
    for( int px = *(int *) numThread; px<width; px += nthreads )
    {
        ...
        for( int py=0; py<height; ++py )
        {
            ...
            vals[px][py][0] = scaled_color[0];
            vals[px][py][1] = scaled_color[1];
            vals[px][py][2] = scaled_color[2];
        }
    }
    return NULL;
}
...
main( int argc, char **argv )
{
    ...
    {
        ids[c] = c;
        if(pthread_create(&threads[c], NULL, renderImg, &ids[c]))
        {
    ...
    {
        if(pthread_join(threads[d], NULL))
        {
    ...
    for(int i = 0; i < width; i++)
    {
        for(int j = 0; j < height; j++)
            printf("%.0f %.0f %.0f\n", vals[i][j][0], vals[i][j][1], vals[i][j][2]);
        printf("\n");
    }
